...
int** CutImage(char* name, int* width, int* height)	//ReadImage를 참조
{
...
{
	int width, height;
	int** img = CutImage(".\\number_big\\9_resize_big.bmp", &width, &height);
	...
	image_rotate = Diagonalmage(img, width, height, bw, bh);	//대각선 긋기
	...
	if (error.minError < error1.minError)
